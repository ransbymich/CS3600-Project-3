...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "If we are in the same position as a ghost"
        if noisyDistance is None:
            "Move it to a jail"
            allPossible[self.getJailPosition()] = 1.0
        else:
            "for all of the moves we're able to make"
            for p in self.legalPositions:
                """Get the emission model value for the distance between pacman and the proposed position,
                    multiply it by our current belief of that position
                """
        ...
        "Submit the beliefs we just calculated to the global variable"
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for old in self.legalPositions:
            ...
            for p, prob in new.items():
                possible[p] += prob * self.beliefs[old]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for loc in self.legalPositions:
                allPossible[loc] = emissionModel[util.manhattanDistance(loc, pacmanPosition)] * oldBelief[loc]
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles:
            particles.append(util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, p))))
